...
async def update_task_output_count(xp: "experiment", delta: int):
    async with xp.central.exitCondition:
        xp.taskOutputQueueSize += delta
        logging.debug(
            "Updating queue size with %d => %d", delta, xp.taskOutputQueueSize
        )
        if xp.taskOutputQueueSize == 0:
            xp.central.exitCondition.notify_all()


class TaskOutputWatcher:
    taskOutputsWorker: "TaskOutputsWorker"

    def __init__(self, job: "Job", subpath: Path, config_method: Callable):
        self.job = job
    ...
    def add_listener(self, listener):
        with self.lock:
            ...
            # Now, at the listener to the list
...
class WatchedOutputsMonitor(FileSystemEventHandler):
    """Watch dynamic outputs generated by one task"""
...
class Job(Resource):
    ...
    def watch_output(self, watched: "WatchedOutput"):
        ...
        if watcher := self.watched_outputs.get(path, None):
            assert (
                watcher.config_method == watched.method
            ), f"Methods differ: {watcher.config_method} and {watched.method}"
        else:
            watcher = TaskOutputWatcher(self, path, watched.method)
            self.watched_outputs[path] = watcher
...
class TaskOutputsWorker(threading.Thread):
    """This worker process dynamic output"""
    ...
    def add(self, watcher, event):
        asyncio.run_coroutine_threadsafe(
            update_task_output_count(self.xp, 1),
    ...
    def run(self):
        ...
        while True:
            ...
            for listener in watcher.listeners:
                ...
                asyncio.run_coroutine_threadsafe(
                    update_task_output_count(self.xp, -1), self.xp.scheduler.loop
                ).result()
